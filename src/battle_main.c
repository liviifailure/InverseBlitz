...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_AQUA_LEADER] = { _("AQUA LEADER"), 25, BALL_POKE },
    ...
    [TRAINER_CLASS_GENTLEMAN] = { _("GENTLEMAN"), 20, BALL_POKE },
    [TRAINER_CLASS_ELITE_FOUR] = { _("ELITE FOUR"), 25, BALL_POKE },
    [TRAINER_CLASS_LEADER] = { _("LEADER"), 25, B_TRAINER_CLASS_POKE_BALLS >= GEN_8 ? BALL_POKE : BALL_POKE },
    ...
    [TRAINER_CLASS_CHAMPION] = { _("CHAMPION"), 50, BALL_POKE },
    ...
    [TRAINER_CLASS_COOLTRAINER_2] = { _("COOLTRAINER"), 5, BALL_POKE },
    ...
    [TRAINER_CLASS_MAGMA_LEADER] = { _("MAGMA LEADER"), 25, BALL_POKE },
...
void SwitchInClearSetData(u32 battler, struct Volatiles *volatilesCopy)
{
    ...
    gBattleStruct->battlerState[battler].wasAboveQuarterHp = gBattleMons[battler].hp > gBattleMons[battler].maxHP / 4;
    ...
    case BATTLE_INTRO_STATE_DRAW_SPRITES:
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            ...
            else
            {
                ...
                gBattleMons[battler].types[1] = GetSpeciesType(gBattleMons[battler].species, 1);
                gBattleMons[battler].types[2] = TYPE_MYSTERY;
                gBattleMons[battler].ability = GetMonAbility(GetBattlerMon(battler));
...
u32 GetBattlerTotalSpeedStat(u32 battler, enum Ability ability, enum HoldEffect holdEffect)
{
    ...
    if (HasWeatherEffect())
    {
        if (ability == ABILITY_SWIFT_SWIM       && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && (gBattleWeather & (B_WEATHER_RAIN | B_WEATHER_THUNDERSTORM)))
            speed *= 2;
    ...
    if (ability == ABILITY_QUICK_FEET && gBattleMons[battler].status1 & STATUS1_ANY)
        speed = (speed * 150) / 100;
    else if (ability == ABILITY_SURGE_SURFER && (IsBattlerWeatherAffected(battler, B_WEATHER_RAIN) || (gBattleWeather & B_WEATHER_THUNDERSTORM)))
        speed *= 2;
