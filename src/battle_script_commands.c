...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_HYPER_POTION,    {   3,   3,   3,   8,   8,   9,  30,   _,   _,   _, } },
    ...
    { ITEM_MAX_POTION,      {   _,   _,   _,   4,   4,   4,   4,   7,   9,   9, } },
         ...
         && !IsBattlerAlive(battler))
        {
            ...
            if (IsOnPlayerSide(battler))
            {
                ...
                if (gBattleResults.playerFaintCounter < 255)
                    ...
                    gSaveBlock1Ptr->playerFaintCounter++;
                    }
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gPlayerParty[*expMonId]))
                {
                    ...
                    if (gBattleStruct->expGetterBattlerId != 0 && (wasSentOut || holdEffect == HOLD_EFFECT_EXP_SHARE))
                    {
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gPlayerParty[*expMonId], MON_DATA_HP) && currLvl != MAX_LEVEL)
            {
                ...
                if (gBattleStruct->expGetterBattlerId != 0)
                {
...
static void RemoveAllWeather(void)
{
    ...
    if (gBattleWeather & B_WEATHER_RAIN)
        ...
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_WEATHER_END_THUNDERSTORM;
    else
