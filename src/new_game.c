...
if (gSaveBlock2Ptr->playerGender != MALE)    
    ...
    SetWarpDestination(MAP_GROUP(MAP_LITTLEROOT_TOWN_MAYS_HOUSE_2F), MAP_NUM(MAP_LITTLEROOT_TOWN_MAYS_HOUSE_2F), -1, 7, 4);
    WarpIntoMap();
    }
else
    {SetWarpDestination(MAP_GROUP(MAP_LITTLEROOT_TOWN_BRENDANS_HOUSE_2F), MAP_NUM(MAP_LITTLEROOT_TOWN_BRENDANS_HOUSE_2F), -1, 1, 4);
...
void NewGameInitData(void)
{
    ...
    ClearSecretBases();
    ClearBerryTrees();
    SetMoney(&gSaveBlock1Ptr->money, 1250);
    SetCoins(0);
    ...
    ResetContestLinkResults();
        ...
        FlagSet(FLAG_SYS_OPPONENT_LEVEL_1);
