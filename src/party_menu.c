...
#define PARTY_MENU_TYPE_ENEMY 13
...
static void InitPartyMenu(u8 menuType, u8 layout, u8 partyAction, bool8 keepCursorPos, u8 messageId, TaskFunc task, MainCallback callback)
{
    ...
    else
    {
        ...
        sPartyMenuInternal->showEnemy = (menuType == PARTY_MENU_TYPE_ENEMY);
        ...
        if (!keepCursorPos)
            gPartyMenu.slotId = 0;
        else if (gPartyMenu.slotId > PARTY_SIZE - 1 || GetMonData(&GetPartyMenuParty()[gPartyMenu.slotId], MON_DATA_SPECIES) == SPECIES_NONE)
            gPartyMenu.slotId = 0;

        if (GetPartyMenuPartyCount() == 0)
            gPartyMenu.slotId = PARTY_SIZE + 1; // Cancel

        gTextFlags.autoScroll = 0;
        if (sPartyMenuInternal->showEnemy)
...
static bool8 AllocPartyMenuBgGfx(void)
{
    ...
    case 2:
        ...
        if (sPartyMenuInternal->showEnemy)
        {
            // Inject orange/gold palette colors for FOE menu
            u16 colors[] = {
                RGB(31, 15, 0),  // 14: Orange Text
                RGB(31, 22, 10), // 73: Light Orange
                RGB(25, 12, 0),  // 74: Dark Orange
                RGB(31, 26, 15), // 75: Bright Gold (Selected)
                RGB(28, 18, 5)   // 76: Darker Bright Gold (Selected)
            };
        ...
        {
            // Inject purple palette colors into unused slots (Player default)
            u16 colors[] = {
...
static void RenderPartyMenuBox(u8 slot)
{
    ...
    else if (GetPartyMenuPartyCount() != 0)
    {
        ...
        else
        {
            ...
            else if (gPartyMenu.menuType == PARTY_MENU_TYPE_ENEMY)
...
{
    if (GetMonData(&GetPartyMenuParty()[slot], MON_DATA_IS_EGG))
    {
        sPartyMenuBoxes[slot].infoRects->blitFunc(sPartyMenuBoxes[slot].windowId, 0, 0, 0, 0, TRUE);
        DisplayPartyPokemonNickname(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot], 0);
    }
    else
    {
        sPartyMenuBoxes[slot].infoRects->blitFunc(sPartyMenuBoxes[slot].windowId, 0, 0, 0, 0, FALSE);
        DisplayPartyPokemonNickname(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot], 0);
        DisplayPartyPokemonLevelCheck(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot], 0);
        DisplayPartyPokemonGenderNidoranCheck(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot], 0);
        DisplayPartyPokemonHPCheck(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot], 0);
        DisplayPartyPokemonMaxHPCheck(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot], 0);
        DisplayPartyPokemonHPBarCheck(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot]);
    }
}
...
static void CreatePartyMonSprites(u8 slot)
{
    ...
    else if (GetMonData(&GetPartyMenuParty()[slot], MON_DATA_SPECIES) != SPECIES_NONE)
    {
        CreatePartyMonIconSprite(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot], slot);
        CreatePartyMonHeldItemSprite(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot]);
        CreatePartyMonPokeballSprite(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot]);
        CreatePartyMonStatusSprite(&GetPartyMenuParty()[slot], &sPartyMenuBoxes[slot]);
    }
}
...
static u8 GetPartyBoxPaletteFlags(u8 slot, u8 animNum)
{
    ...
    if (GetMonData(&GetPartyMenuParty()[slot], MON_DATA_HP) == 0)
        palFlags |= PARTY_PAL_FAINTED;
    if (PartyBoxPal_ParnterOrDisqualifiedInArena(slot) == TRUE || sPartyMenuInternal->showEnemy)
...
static void Task_ClosePartyMenuAndSetCB2(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        if (gPartyMenu.menuType == PARTY_MENU_TYPE_IN_BATTLE && !sPartyMenuInternal->showEnemy)
...
static u16 PartyMenuButtonHandler(s8 *slotPtr)
{
    ...
    if (movementDir && GetPartyMenuPartyCount() != 0)
    {
...
static void UpdatePartySelectionSingleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    case MENU_DIR_UP:
        ...
        {
            *slotPtr = GetPartyMenuPartyCount() - 1;
        }
    ...
    case MENU_DIR_DOWN:
        ...
        {
            if (*slotPtr == GetPartyMenuPartyCount() - 1)
            {
                if (sPartyMenuInternal->chooseHalf)
                    *slotPtr = PARTY_SIZE;
                else
...
static void UpdatePartySelectionDoubleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    case MENU_DIR_RIGHT:
        if (*slotPtr == 0)
        {
            ...
            {
                if (GetMonData(&GetPartyMenuParty()[3], MON_DATA_SPECIES) != SPECIES_NONE)
                    *slotPtr = 3;
            }
            else if (GetMonData(&GetPartyMenuParty()[2], MON_DATA_SPECIES) != SPECIES_NONE)
            {
                *slotPtr = 2;
            }
        }
        else if (*slotPtr == 1)
        {
            ...
            {
                if (GetMonData(&GetPartyMenuParty()[5], MON_DATA_SPECIES) != SPECIES_NONE)
                    *slotPtr = 5;
            }
            else if (GetMonData(&GetPartyMenuParty()[4], MON_DATA_SPECIES) != SPECIES_NONE)
            {
                *slotPtr = 4;
            }
        }
...
static s8 GetNewSlotDoubleLayout(s8 slotId, s8 movementDir)
{
    while (TRUE)
    {
        ...
        if ((u8)slotId >= PARTY_SIZE)
            return -1;
        if (GetMonData(&GetPartyMenuParty()[slotId], MON_DATA_SPECIES) != SPECIES_NONE)
            return slotId;
    }
}
...
//        }}
    if (!(palFlags & (PARTY_PAL_NO_MON | PARTY_PAL_FAINTED | PARTY_PAL_MULTI_ALT))
        && ((VarGet(VAR_BADGE_COUNT) >= 8 && (
         ...
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_GRAVELER_ALOLA
         ...
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_RHYDON
        )) || (VarGet(VAR_BADGE_COUNT) >= 4 && (
            GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_VULPIX_ALOLA
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_BRAMBLIN
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_CUBCHOO
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_GRIMER
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_SLUGMA
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_GLAMEOW
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_MAREANIE
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_SCRAGGY
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_FOONGUS
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_BINACLE
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_OMANYTE
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_KABUTO
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_LILEEP
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_ANORITH
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_FERROSEED
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_MIENFOO
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_VULLABY
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_GLIGAR
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_PANPOUR
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_PANSAGE
         ...
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_PAWNIARD
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_SINISTEA
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_SHELMET
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_DURALUDON
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_GIMMIGHOUL
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_KARRABLAST
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_POIPOLE
         ...
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_SKITTY
         ...
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_DREEPY
         ...
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_KLINK
         ...
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_CATERPIE
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_METAPOD
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_WURMPLE
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_SILCOON
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_CASCOON
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_KRICKETOT
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_NUZLEAF
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_SENTRET
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_WYNAUT
         || GetMonData(&gPlayerParty[menuBox->windowId], MON_DATA_SPECIES) == SPECIES_BIDOOF
...
{
    struct Pokemon *mon = &GetPartyMenuParty()[gPartyMenu.slotId];
    u16 item;
...
static void CB2_ShowPokemonSummaryScreen(void)
{
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_IN_BATTLE)
    {
        ...
        ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, GetPartyMenuParty(), gPartyMenu.slotId, GetPartyMenuPartyCount() - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
    }
    else if (gPartyMenu.menuType == PARTY_MENU_TYPE_CHOOSE_HALF || gPartyMenu.menuType == PARTY_MENU_TYPE_ENEMY)
    {
        ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, GetPartyMenuParty(), gPartyMenu.slotId, GetPartyMenuPartyCount() - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
    }
    else
    {
        ShowPokemonSummaryScreen(SUMMARY_MODE_NORMAL, GetPartyMenuParty(), gPartyMenu.slotId, GetPartyMenuPartyCount() - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
    }
}
...
static void Task_HealPokemon(u8 taskId)
{
    ...
    {
        bool8 hadStatus = TRUE;
        SetMonData(mon, MON_DATA_STATUS, &statusNone);
        SetPartyMonAilmentGfx(mon, &sPartyMenuBoxes[gPartyMenu.slotId]);
        if (hadStatus && gSprites[sPartyMenuBoxes[gPartyMenu.slotId].statusSpriteId].invisible)
            DisplayPartyPokemonLevelCheck(mon, &sPartyMenuBoxes[gPartyMenu.slotId], 1);
    }
    MonRestorePP(mon);
